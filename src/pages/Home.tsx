    ...
    <div className="min-h-full" style={{ background: "var(--color-bg)" }}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 h-14 flex items-center justify-between px-6 border-b"
        ...
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate("/")}>
          ...
          <span className="text-sm font-semibold" style={{ color: "var(--color-text)" }}>心流创坊</span>
        </div>
      ...
      <section className="relative min-h-[55vh] flex flex-col items-center justify-center overflow-hidden pt-14">
        ...
        <motion.div className="relative z-10 text-center px-4 max-w-2xl mx-auto" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <span className="text-sm" style={{ color: "#4A90E2" }}>心流创坊 · AI情绪创作疗愈平台</span>
          </div>
          ...
          <div className="max-w-md mx-auto mb-4">
            <div className="flex items-center gap-2 rounded-xl px-4 py-2.5 border bg-white/95"
              ...
              <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter" && chatInput.trim()) navigate("/module/copywriting"); }}
                placeholder="和心流伴侣聊聊..." className="flex-1 bg-transparent text-sm outline-none placeholder:text-slate-400" style={{ color: "var(--color-text)" }} />
              <button onClick={() => { if (chatInput.trim()) navigate("/module/copywriting"); }}
                className="p-1.5 rounded-lg" style={{ background: chatInput.trim() ? "linear-gradient(135deg, #4A90E2, #6BA3E0)" : "#E2E8F0", color: chatInput.trim() ? "#fff" : "#94A3B8" }}>
